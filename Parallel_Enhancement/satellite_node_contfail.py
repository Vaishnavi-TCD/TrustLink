...
ROUND_ROBIN_INDEX = 0  # Index for round-robin load balancing
    ...
    while not message_queue.empty():
        ...
        try:
            if destination in ROUTING_TABLE:
                ...
                # Default to round-robin neighbor selection for load balancing
                neighbor = neighbors[ROUND_ROBIN_INDEX % len(neighbors)]
                ROUND_ROBIN_INDEX += 1
                forward_to_neighbor(json.dumps(msg), neighbor)
        except Exception as e:
...
def respond_to_vehicle(addr, vehicle_id):
    ...
    try:
        ...
        print(f"Sent ACK to Vehicle {vehicle_id} at {addr[0]}:{ACK_LISTEN_PORT}")
    except Exception as e:
