...
import queue
import sys
from encryption_util import encrypt_message, decrypt_message  # Import encryption utilities
...
def create_encrypted_message(msg_type, source, destination, payload=None):
    """Create and encrypt a formatted message."""
    plain_message = json.dumps({
        "type": msg_type,
        "source": source,
    ...
    return encrypt_message(plain_message)
...
def broadcast_presence():
    ...
    while True:
        try:
            message = create_encrypted_message(
...
def send_encrypted_ack(sock, addr, source):
    """Send an encrypted acknowledgment to the sender."""
    ack_message = create_encrypted_message("ack", f"satellite_{SATELLITE_PORT}", source, {"status": "received"})
...
def forward_encrypted_message(message):
    """Forward encrypted message based on routing table."""
    destination = message["destination"]
    next_hop = get_next_hop(destination)
    if next_hop:
        forward_to_neighbor(encrypt_message(json.dumps(message)), next_hop["ip"], next_hop["port"])
    else:
...
def forward_to_neighbor(data, ip, port):
    """Forward encrypted message to a specific neighbor."""
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
...
def handle_connection(sock, addr, data):
    """Handle an individual connection."""
    try:
        start_time = time.time()
        decrypted_message = json.loads(decrypt_message(data.decode()))
        logging.info(f"Handling message: {decrypted_message}")
        if is_duplicate(decrypted_message["id"]):
            logging.warning(f"Duplicate message detected: {decrypted_message['id']}")
            return
        if decrypted_message["type"] == "data":
            forward_encrypted_message(decrypted_message)
            send_encrypted_ack(sock, addr, decrypted_message["source"])
        elif decrypted_message["type"] == "announcement":
            update_routing_table(decrypted_message, addr)
        with metrics_lock:
...
def main():
    ...
    while True:
        try:
            ...
            logging.info(f"Received encrypted data from {addr}")
            connection_queue.put((addr, data))
        except Exception as e:
