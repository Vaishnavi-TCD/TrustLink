...
def discover_satellites():
    ...
    while time.time() - start_time < SATELLITE_DISCOVERY_TIMEOUT:
        try:
            ...
            if decrypted_message.get("type") == "announcement" and "port" in decrypted_message["payload"]:
                discovered[decrypted_message["source"]] = {
                    "ip": addr[0],
                    "port": decrypted_message["payload"]["port"]
                }
                logging.info("Discovered Satellite %s on IP %s Port %s" % (
                    decrypted_message["source"], addr[0], decrypted_message["payload"]["port"]))
                with metrics_lock:
...
def send_encrypted_data(sock, satellite_ip, satellite_port, message):
    """Send encrypted data to a satellite and wait for acknowledgment."""
    for attempt in range(MAX_RETRIES):
        try:
            ...
            logging.info("Sent encrypted data to %s:%s." % (satellite_ip, satellite_port))
            with metrics_lock:
                metrics["total_messages_sent"] += 1
 
            sock.settimeout(ACK_TIMEOUT)
            ack, addr = sock.recvfrom(1024)
            ack_message = json.loads(decrypt_message(ack.decode()))
...
def main():
    ...
    try:
        while not shutdown_flag.is_set():
            ...
            encrypted_message = create_encrypted_message("data", vehicle_id, "satellite", {"gps": gps_data})
 
            selected_satellite = select_best_satellite(satellites)
            if selected_satellite:
                success = send_encrypted_data(
                    sock, selected_satellite["ip"], selected_satellite["port"], encrypted_message
                )
                if success:
